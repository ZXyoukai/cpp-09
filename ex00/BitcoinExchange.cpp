...
std::list<ttype> split(const char *line, char _delim)
{
...
BitcoinExchange::BitcoinExchange(const std::string &line, const char del)
{
    ...
    // Extrair os dois elementos da lista
    std::list<std::string>::iterator it = date_rate.begin();
    std::string dateStr = *it++;
    std::string rateStr = *it;
    
    data = split<int>(dateStr.c_str(), '-');
    if (data.size() != 3)
    {
        excpt = "bad input => " + line;
        return ;
    ...
    std::list<int>::iterator dataIt = data.begin();
    _date.year = *dataIt++;
    _date.moth = *dataIt++;
    _date.day = *dataIt;
    
    std::list<float> rateList = split<float>(rateStr.c_str(), '\n');
    if (rateList.empty())
    {
        excpt = "bad input => " + line;
        return ;
